...
export const nonprofitSearch = tool('nonprofit_search', {
  ...
  enrichment: {
    notice: z.string().optional().describe('Present when the page carried no organizations.'),
  },
  errors: [
    {
      reason: 'upstream_error',
      code: JsonRpcErrorCode.ServiceUnavailable,
      when: 'ProPublica returns a 5xx or the request fails at the network layer',
      retryable: true,
  ...
  async handler(input, ctx) {
    ...
    // A zero-hit search succeeded — return an empty list plus a notice, never an error.
    if (orgs.length === 0) ctx.enrich.notice(`No organizations matched "${input.query}".`);
